...
//  we'll prepare this later
    ...
    vaultwidth = ROW;
    // vaultwidth = 11;
// console.log(vaultwidth, VAULTORDER);
	///wawa init bug
	init_totals();
...
// Mule object
	...
	var magicWord = "";
        if (['kongregate', 'steamworks', 'kabam'].indexOf(platform) >= 0) {
            CR.secret = pass
	    magicWord = "secret=" + pass
        } else {
            CR.password = pass
	    magicWord = "password=" + pass
        }
		...
		$.ajax({
			...
			method: "POST",
			data: "guid=" + this.guid.replace("+", "%2B") + "&" + magicWord + "&clientToken=0",
			complete: myCallBack,
    ...
    Mule.prototype.parse = function(data, skipCacheWrite) {
		...
		///wawa start
		var arr = [];
		var ClassList = { Rogue: {Num: 0}, Archer: {Num: 0}, Wizard: {Num: 0}, Priest: {Num: 0}, Warrior: {Num: 0}, Knight: {Num: 0}, Paladin: {Num: 0}, Assassin: {Num: 0}, Necromancer: {Num: 0}, Huntress: {Num: 0}, Mystic: {Num: 0}, Trickster: {Num: 0}, Sorcerer: {Num: 0}, Ninja: {Num: 0}, Samurai: {Num: 0}, Bard: {Num: 0}, Summoner: {Num: 0} };
		var CharNumData = [];
		...
		// {
			...
			CharNumData.push('<div class="flex-container noFlexAutoJustify noFlexAutoAlign ml10 mr10" style="flex-flow:column; flex-shrink:0; flex-basis:content; align-self:flex-end; justify-content:flex-end; height:100%"><div class="flex-container" style="flex-shrink:0; justify-content:flex-end">Number of Chars Per Class : </div><div class="flex-container" style="flex-shrink:0; justify-content:flex-end">Highest Fame Per Class : </div></div>');
			for (var i in ClassList) {
				CharNumData.push('<div class="flex-container ml10 mr10" style="flex-flow:column"><div class="item"><img src="lib/' + i + '.png"></img><div style="">' + ClassList[i]['Num'] + '</div></div><div class="flex-container">' + (d.Account.Stats.ClassStats[stupidvar] ? d.Account.Stats.ClassStats[stupidvar].BestFame : 0) + '</div></div>');
				stupidvar += 1;
			}
			stupidvar = 0;
			...
			for (var i = 0; i < chests.length; i++) {
				...
				if (chest = chests[i].split(',')) {
					for (var j = 0; j < 8; j++) {
						if (chest[j] !== '-1' && chest[j] !== undefined && Totitems !== undefined) {
			...
			for ( var z = 0; z < CountingCharList.length; z++ ) {
				...
				for ( var tec = 0; tec < eqc.length; tec++ ) {
					if (eqc[tec] !== '-1' && eqc[tec] !== undefined && Totitems !== undefined) {
