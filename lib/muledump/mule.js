...
// dom snippet generators
    ...
    function addreloader(mule, target) {
        ...
        if (mule.data) {
			...
			// console.log(updated.toString().substring(0,15));
        }
		if ((updated.toString().substring(8,10)) > 26)
		{
			if (check == 0)
			{
...
// Mule object
    ...
    Mule.prototype.parse = function(data, skipCacheWrite) {
		...
		///wawa start
		var arr = [];
		var ClassList = { Rogue: {Num: 0}, Archer: {Num: 0}, Wizard: {Num: 0}, Priest: {Num: 0}, Warrior: {Num: 0}, Knight: {Num: 0}, Paladin: {Num: 0}, Assassin: {Num: 0}, Necromancer: {Num: 0}, Huntress: {Num: 0}, Mystic: {Num: 0}, Trickster: {Num: 0}, Sorcerer: {Num: 0}, Ninja: {Num: 0}, Samurai: {Num: 0} };
		var CharNumData = [];
        ...
        if ( this.loginOnly === false ) {
			...
			///dungeon stats
			var dungeonbutton = $('<div class="button"></div>');
			dungeonbutton.text('\u2713');
            ...
            var $name = $('<div>').addClass('name').html(d.Account.Name + ' : ' + (d.Char.length != undefined ? d.Char.length + ' / ' + ((data.query.results.other) ? data.query.results.other.maxCharNum : 'N/A') + ' Chars' : '1 / ' + ((data.query.results.other) ? data.query.results.other.maxCharNum : 'N/A') + ' Char') || '(No Name) : ' + (d.Char.length != undefined ? d.Char.length + ' / ' + ((data.query.results.other) ? data.query.results.other.maxCharNum : 'N/A') + ' Chars' : '1 / ' + ((data.query.results.other) ? data.query.results.other.maxCharNum : 'N/A') + ' Char'));
            addstar(this.dom, d);
			...
			var $AccFame = $('<div>').addClass('famegold').html(this.data.query.results.Chars.Account.Stats.Fame + '  ');
			var $AccGold = $('<div>').addClass('famegold').html(this.data.query.results.Chars.Account.Credits + '  ');
			$AccFame.append('<img class="imgfamegold" src="lib/Fame.png"></img></div>');
			$AccFame.appendTo(this.dom);
			$AccGold.append('<img class="imgfamegold" src="lib/Gold.png"></img></div>');
			$AccGold.appendTo(this.dom);
        ...
        for (var i = 0; i < carr.length; i++) {
            ...
            if ( this.loginOnly === false && !this.opt('shrink') && this.opt('stats') ) {
                    ...
                    if (stt === 'base') {
						...
						if (s < cl[3][t]) {
							...
							if (t < 2) {
								...
								StatData.push('<div class="' + (avgd > 0 ? 'good' : (avgd < 0 ? 'bad' : 'avg')) + ((Math.abs(avgd) > 14) ? ' very' : '') + '" style="font-size:1.3em; font-weight:700">' + (avgd > 0 ? '+' : '') + avgd + ' From Average ( ' + (avgd > 0 ? '+' : '') + avgd / 5 + ' )</div><div style="font-size:1.3em; font-weight:700">' + l2m + ' Left To Max</div>');
							}
							else {
				...
				var l2mHtml = [];
				var LifePot = window.items[2793];
				var ManaPot = window.items[2794];
				var AttPot = window.items[2591];
				var DefPot = window.items[2592];
				var SpdPot = window.items[2593];
				var DexPot = window.items[2636];
				var VitPot = window.items[2612];
				var WisPot = window.items[2613];
				
				l2mHtml.push('<div class="flex-container" style="flex-direction: row">Pots Left To Max :</div><div class="flex-container" style="flex-direction: row">' + 
					(l2mArray[0] != 0 ? '<div class="flex-container item noFlexAutoWidth" style="background-position: -' + LifePot[3] + 'px -' + LifePot[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + l2mArray[0] + '</div></div>' : '') + 
					(l2mArray[1] != 0 ? '<div class="flex-container item noFlexAutoWidth" style="background-position: -' + ManaPot[3] + 'px -' + ManaPot[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + l2mArray[1] + '</div></div>' : '') + 
					(l2mArray[2] != 0 ? '<div class="flex-container item noFlexAutoWidth" style="background-position: -' + AttPot[3] + 'px -' + AttPot[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + l2mArray[2] + '</div></div>' : '') + 
					(l2mArray[3] != 0 ? '<div class="flex-container item noFlexAutoWidth" style="background-position: -' + DefPot[3] + 'px -' + DefPot[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + l2mArray[3] + '</div></div>' : '') + 
					(l2mArray[4] != 0 ? '<div class="flex-container item noFlexAutoWidth" style="background-position: -' + SpdPot[3] + 'px -' + SpdPot[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + l2mArray[4] + '</div></div>' : '') + 
					(l2mArray[5] != 0 ? '<div class="flex-container item noFlexAutoWidth" style="background-position: -' + DexPot[3] + 'px -' + DexPot[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + l2mArray[5] + '</div></div>' : '') + 
					(l2mArray[6] != 0 ? '<div class="flex-container item noFlexAutoWidth" style="background-position: -' + VitPot[3] + 'px -' + VitPot[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + l2mArray[6] + '</div></div>' : '') + 
					(l2mArray[7] != 0 ? '<div class="flex-container item noFlexAutoWidth" style="background-position: -' + WisPot[3] + 'px -' + WisPot[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + l2mArray[7] + '</div></div>' : '') + '</div>');
				
				// console.log(FullID, l2mHtml);
            }
            ...
            */
			...
			var stupidvar2 = 0;
			for (m in shortdungeonnames) {
				DungeonStats[stupidvar2].Number += st[m];
				if (m > 12) {
					if (st[m] != 0) {
		...
		var stupidvard = 0;
		var stupidvars = 0;
		CharNumData.push('<div class="flex-container noFlexAutoWidth" style="flex-flow:column; font-size:1.3em; font-weight:700"><div class="flex-container noFlexAutoWidth" style="flex-flow:row">');
		for (var i in ClassList) {
			CharNumData.push('<div class="flex-container mr10" style="flex-flow:column"><div class="item"><img src="lib/' + i + '.png"></img><div style="">' + ClassList[i]['Num'] + '</div></div><div class="flex-container">' + (d.Account.Stats.ClassStats[stupidvar] ? d.Account.Stats.ClassStats[stupidvar].BestFame : 0) + '</div></div>');
			stupidvar += 1;
		}
		CharNumData.push('</div><div class="flex-container">\xa0</div><div class="flex-container">Total Base Fame : ' + NumberFormat(BaseFameTotal, ',') + ' Fame</div><div class="flex-container" style="flex-flow:column">Total Dead Fame : ' + NumberFormat(DeadFameTotal, ',') + ' Fame</div><div>\xa0</div>');
		
		var SlotCoupon = window.items[18522];
		var ChestCoupon = window.items[18516];
		var STShard = window.items[8643];
		var QuestChest = window.items[3468];
		var EpicQuestChest = window.items[3470];
		
		var LootTier = window.items[3176];
		var LootDrop = window.items[3177];
		var Clover = window.items[3266];
		var MaxLvl = window.items[6731];
		...
		var Totitems = [];
		var Totitems = (typeof gifts === 'string') ? gifts.split(',') : undefined;
        ...
        for (var tg = 0; tg < Totitems.length; tg++) {
			...
			/// slot coupons
			if ( Totitems[tg] === '18522') {
				SlotCouponNumber += 1;
			}
			if ( Totitems[tg] === '18523') {
				SlotCouponNumber += 2;
			}
			if ( Totitems[tg] === '18524') {
				SlotCouponNumber += 3;
			}
			if ( Totitems[tg] === '18525') {
				SlotCouponNumber += 4;
			}
			if ( Totitems[tg] === '18526') {
				SlotCouponNumber += 5;
			}
			
			/// chest coupons
			if ( Totitems[tg] === '18516') {
				ChestCouponNumber += 1;
			}
			if ( Totitems[tg] === '18517') {
				ChestCouponNumber += 2;
			}
			if ( Totitems[tg] === '18518') {
				ChestCouponNumber += 3;
			}
			if ( Totitems[tg] === '18519') {
				ChestCouponNumber += 4;
			}
			if ( Totitems[tg] === '18520') {
				ChestCouponNumber += 5;
			}
			...
			/// keys
			// console.log(window.items[Totitems[tg]][0]);
			if (window.items[Totitems[tg]] != undefined) {
				...
				/// cloth
				else if(itemtype == "Cloth" || itemtype2 == "Cloth") {
				...
				/// dye
				else if(itemtype == "Dye" || itemtype2 == "Dye") {
		...
		/// pots
		if (l2mArray) {
			if (l2mArray[0] != 0 || l2mArray[1] != 0 || l2mArray[2] != 0 || l2mArray[3] != 0 || l2mArray[4] != 0 || l2mArray[5] != 0 || l2mArray[6] != 0 || l2mArray[7] != 0) {
				CharNumData.push('<div class="flex-container" style="flex-direction: row;"><div class="flex-container" style="flex-direction: column">' + l2mHtml + '</div>');
			}
			else {
		...
		/// items + buffs
		CharNumData.push('<div class="flex-container" style="flex-direction: column"><div class="flex-container" style="flex-direction: row">Items :</div><div class="flex-container" style="flex-direction: row">' +
			'<div class="flex-container item noFlexAutoWidth" style="background-position: -' + SlotCoupon[3] + 'px -' + SlotCoupon[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + SlotCouponNumber + '</div></div>' + 
			'<div class="flex-container item noFlexAutoWidth" style="background-position: -' + ChestCoupon[3] + 'px -' + ChestCoupon[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + ChestCouponNumber + '</div></div>' + 
			'<div class="flex-container item noFlexAutoWidth" style="background-position: -' + STShard[3] + 'px -' + STShard[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + STShardNumber + '</div></div>' + 
			'<div class="flex-container item noFlexAutoWidth" style="background-position: -' + QuestChest[3] + 'px -' + QuestChest[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + QuestChestNumber + '</div></div>' + 
			'<div class="flex-container item noFlexAutoWidth" style="background-position: -' + EpicQuestChest[3] + 'px -' + EpicQuestChest[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + EpicQuestChestNumber + '</div></div></div></div>');
		
		CharNumData.push('<div class="flex-container" style="flex-direction: column"><div class="flex-container" style="flex-direction: row">Buffs :</div><div class="flex-container" style="flex-direction: row">' +
			'<div class="flex-container item noFlexAutoWidth" style="background-position: -' + LootDrop[3] + 'px -' + LootDrop[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + LootDropNumber + '</div></div>' + 
			'<div class="flex-container item noFlexAutoWidth" style="background-position: -' + LootTier[3] + 'px -' + LootTier[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + LootTierNumber + '</div></div>' + 
			'<div class="flex-container item noFlexAutoWidth" style="background-position: -' + Clover[3] + 'px -' + Clover[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + CloverNumber + '</div></div>' + 
			'<div class="flex-container item noFlexAutoWidth" style="background-position: -' + MaxLvl[3] + 'px -' + MaxLvl[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + MaxLvlNumber + '</div></div></div></div></div>'); 
		
		/// keys + skins
		CharNumData.push('<div>\xa0</div><div class="flex-container" style="flex-direction: row"><div class="flex-container" style="flex-direction: column"><div class="flex-container" style="flex-direction: row;">Keys :</div><div class="flex-container mr10" style="flex-direction: row">');
		for (var i in KeyList) {
			if (stupidvark === 20) {
				CharNumData.push('</div><div class="flex-container mr10" style="flex-direction: row">');
				stupidvark = 0;
			}
			CharNumData.push('<div class="flex-container item noFlexAutoWidth" style="background-position: -' + window.items[i][3] + 'px -' + window.items[i][4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + KeyList[i] + '</div></div>');
			stupidvark++;
		}
		CharNumData.push('</div></div>'); 
		
		CharNumData.push('<div class="flex-container" style="flex-direction: column"><div class="flex-container" style="flex-direction: row;">Skins :</div><div class="flex-container" style="flex-direction: row">');
		for (var i in SkinList) {
			if (stupidvars === 20) {
				CharNumData.push('</div><div class="flex-container" style="flex-direction: row">');
				stupidvars = 0;
			}
			CharNumData.push('<div class="flex-container item noFlexAutoWidth" style="background-position: -' + window.items[i][3] + 'px -' + window.items[i][4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + SkinList[i] + '</div></div>');
			stupidvars++;
		}
		CharNumData.push('</div></div></div>'); 
		
		/// clothes + dyes	
		CharNumData.push('<div>\xa0</div><div class="flex-container" style="flex-direction: row"><div class="flex-container" style="flex-direction: column"><div class="flex-container" style="flex-direction: row">Clothes :</div><div class="flex-container mr10" style="flex-direction: row">');
		for (var i in ClothList) {
			if (stupidvarc === 20) {
				CharNumData.push('</div><div class="flex-container mr10" style="flex-direction: row">');
				stupidvarc = 0;
			}
			CharNumData.push('<div class="flex-container item noFlexAutoWidth" style="background-position: -' + window.items[i][3] + 'px -' + window.items[i][4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + ClothList[i] + '</div></div>');
			stupidvarc++;
		}
		CharNumData.push('</div></div>'); 
		
		CharNumData.push('<div class="flex-container" style="flex-direction: column"><div class="flex-container">Dyes :</div><div class="flex-container" style="flex-direction: row">');
		for (var i in DyeList) {
			if (stupidvard === 20) {
				CharNumData.push('</div><div class="flex-container" style="flex-direction: row">');
				stupidvard = 0;
			}
			CharNumData.push('<div class="flex-container item noFlexAutoWidth" style="background-position: -' + window.items[i][3] + 'px -' + window.items[i][4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"><div style="">' + DyeList[i] + '</div></div>');
			stupidvard++;
		}
		CharNumData.push('</div></div></div>'); 
		...
		///dungeon stats
		var DungeonStatsHtml1 = [];
		var DungeonStatsHtml2 = [];
		DungeonStatsHtml.push('<div style="display:flex; flex-direction: row"><div class="flex-container noFlexAutoWidth noFlexAutoAlign" style="flex-direction: column"><div class="flex-container noFlexAutoAlign" style="justify-content:flex-start">')
		DungeonStatsHtml1.push('<div class="flex-container noFlexAutoWidth noFlexAutoAlign" style="flex-direction: column">')
		DungeonStatsHtml2.push('<div class="flex-container noFlexAutoWidth noFlexAutoAlign" style="flex-direction: column">')
		for (var i = 0; i < DungeonStats.length; i++) {
			DungeonStatsHtml1.push('<div class="statItem" style="font-size:1.15em; align-self:flex-start">' + DungeonStats[i].Dungeon + ' : </div>');
			DungeonStatsHtml2.push('<div class="statItem" style="font-size:1.15em; align-self:flex-end">' + DungeonStats[i].Number + '</div>');
		}
		DungeonStatsHtml.push(DungeonStatsHtml1.join(' ') + '</div>' + DungeonStatsHtml2.join(' ') + '</div></div></div>');
		
		setuptools.app.muledump.tooltip(dungeonbutton, 'statContainer autoHeight', DungeonStatsHtml.join(''));
		...
		for (var i = 0; i < window.items[8848].length; i++) {
			console.log(i, window.items[8848][i]);
		}
		console.log(window.items[8848]);
