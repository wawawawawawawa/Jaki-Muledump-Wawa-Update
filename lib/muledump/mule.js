...
// Mule object
	...
	if (vaultlayout == "3" || vaultlayout == "4")
        ...
        if ( d.Account.Vault && this.totals.cache === false ) {
            ...
            for ( var tv = 0; tv < chests.length; tv++ ) {
                ...
                if (chest = chests[tv].split(','))
					...
					{
						let itemid = chest[tv2] || '-1';
						let res = itemid.split('#')[0]
						if(res !== null) itemid = res;
						this.totals.count(itemid, 'vaults');
					}
		...
		if (this.opt('vaultlayout') === "4" || this.opt('vaultlayout') === "3")
        ...
        if (this.opt('gifts')) {
            ...
            if ( Array.isArray(items) === true ) {
                ...
                while (items.length) {
                    ...
                    garr.push(makechest(items, 'gifts', true));
                    items = items.slice(8);
                }
                if ( this.loginOnly === false && !this.opt('shrink') ) maketable('gifts', garr, ( (setuptools.data.config.giftChestWidth === 0) ? (setuptools.data.config.rowlength / 2) : (setuptools.data.config.giftChestWidth / 2) )).appendTo(this.dom);
            }
