...
// Mule object
    ...
    Mule.prototype.parse = function(data, skipCacheWrite) {
        ...
        if ( this.loginOnly === false ) {
			...
			var $name = $('<div>').addClass('name').html(d.Account.Name + ' : ' + ((d.Char.length != undefined ? d.Char.length + ' / ' + ((data.query.results.other) ? data.query.results.other.maxCharNum : 'N/A') + ' Chars & ' + d.Account.Vault.Chest.length + ' Vaults' : '1 / ' + ((data.query.results.other) ? data.query.results.other.maxCharNum : 'N/A') + ' Char & ' + d.Account.Vault.Chest.length + ' Vaults') || '(No Name) : ' + (d.Char.length != undefined ? d.Char.length + ' / ' + ((data.query.results.other) ? data.query.results.other.maxCharNum : 'N/A') + ' Chars & ' + d.Account.Vault.Chest.length + ' Vaults' : '1 / ' + ((data.query.results.other) ? data.query.results.other.maxCharNum : 'N/A') + ' Char & ' + d.Account.Vault.Chest.length + ' Vaults')));
		...
		var StatId = [2793, 2794, 2591, 2592, 2593, 2636, 2612, 2613];
		var PotList = {Count: 0}
		var DungeonStats = [];
		var STShardNumber = 0;
		...
		CharNumData.push('<div class="flex-container mr10" style="flex-flow:column; flex-shrink:0"><div class="flex-container" style="flex-flow:row;  flex-shrink:0">Total Base Fame : ' + NumberFormat(BaseFameTotal, ',') + ' Fame</div><div class="flex-container" style="flex-flow:row;  flex-shrink:0">Total Dead Fame : ' + NumberFormat(DeadFameTotal, ',') + ' Fame<div></div></div></div></div>');
		...
		if (PotList['Count'] > 0) {
			CharNumData.push('<div class="flex-container" style="flex-direction: column; border: solid 2px grey"><div class="flex-container" style="flex-direction: row;">Pots Left to Max :</div><div class="flex-container" style="flex-direction: row">');
