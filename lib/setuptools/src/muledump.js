 ...
 * @param {string} guid
 * @returns {string}
 * Generate a one click login url OLD
 */
setuptools.app.muledump.mulelinkold = function(guid) {
    ...
    // return the link
	return 'muledump:' + sodium.to_hex(guid) + '-' + sodium.to_hex(window.accounts[guid]) + '-' + 1 + '-' + 0;
};
...
setuptools.app.muledump.mulelink = function(guid) {
	...
	var separator = '_dont_you_dare_use_this_separator_'
    if (setuptools.state.loaded === true && setuptools.data.accounts.accounts[guid].testing === true)
    return;

    return 'muledump:' + sodium.to_hex(guid) + separator + sodium.to_hex(window.accounts[guid]) + separator + 0 + separator + 'data:{platform:Deca,password:' + btoa(window.accounts[guid]) + ',guid:' + btoa(guid) + ',env:4}' + separator + guid + separator + window.accounts[guid];
};
...
setuptools.app.muledump.drawPortrait = function(c, $c) {
	...
	var FameItem = window.items[24102];
	var XPItem = window.items[3139];
    if ( $c instanceof jQuery ) {
	
				var boosts = [c.XpBoosted, c.XpTimer, c.LDTimer, c.LTTimer];
				if (boosts[0] !== "0" || boosts[1] !== "0" || boosts[2] !== "0" || boosts[3] !== "0") {
					var boost = $('<div class="flex-container noFlexAutoWidth boost" style="flex-flow:column"></div>');
					boost.append($('<img style="width:12px; height:15px" src="lib/media/boost.png"></img>'));
					var boostHtml = '<div class="flex-container noFlexAutoWidth noFlexAutoAlign" style="color:#fff; flex-flow:column">' 
							+ (( c.XpBoosted !== "0" ) ? '<div class="flex-container noFlexAutoAlign" style="justify-content:flex-start">' 
								+ '<div class="flex-container item noFlexAutoWidth" style="background-position: -' + XPBoost[3] + 'px -' + XPBoost[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"></div>'
								+ '<div class="flex-container noFlexAutoWidth" style="flex-flow:column; flex-shrink:0; margin-right:5px">' 
									+ '<div style="font-weight:700; font-size:1.5em; align-self:flex-start">XP Boost : </div>' 
								+ '</div>'
								+ '<div class="flex-container noFlexAutoAlign" style="flex-direction:column">' 
									+ '<div style="font-weight:700; font-size:1.5em; align-self:flex-end; margin-right:5px;">' + Math.floor(Number(c.XpTimer)/60) + ' minutes ' + (Number(c.XpTimer) - ((Math.floor(Number(c.XpTimer)/60)) * 60)) + ' seconds</div>' 
							+ '</div></div>' : '') 
								
							+ (( c.LDTimer !== "0" ) ? '<div class="flex-container noFlexAutoAlign" style="justify-content:flex-start">' 
								+ '<div class="flex-container item noFlexAutoWidth" style="background-position: -' + LDBoost[3] + 'px -' + LDBoost[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"></div>'
								+ '<div class="flex-container noFlexAutoWidth" style="flex-flow:column; flex-shrink:0; margin-right:5px">' 
									+ '<div style="font-weight:700; font-size:1.5em; align-self:flex-start">Loot Drop : </div>' 
								+ '</div>'
								+ '<div class="flex-container noFlexAutoAlign" style="flex-direction:column">' 
									+ '<div style="font-weight:700; font-size:1.5em; align-self:flex-end; margin-right:5px;">' + Math.floor(Number(c.LDTimer)/60) + ' minutes ' + (Number(c.LDTimer) - ((Math.floor(Number(c.LDTimer)/60)) * 60)) + ' seconds</div>' 
							+ '</div></div>' : '') 
								
							+ (( c.LTTimer !== "0" ) ? '<div class="flex-container noFlexAutoAlign" style="justify-content:flex-start">' 
								+ '<div class="flex-container item noFlexAutoWidth" style="background-position: -' + LTBoost[3] + 'px -' + LTBoost[4] + 'px; border: solid 1px #000; align-self:center; margin-right:5px; flex-shrink:0;"></div>'
								+ '<div class="flex-container noFlexAutoWidth" style="flex-flow:column; flex-shrink:0; margin-right:5px">' 
									+ '<div style="font-weight:700; font-size:1.5em; align-self:flex-start">Loot Tier : </div>' 
								+ '</div>'
								+ '<div class="flex-container noFlexAutoAlign" style="flex-direction:column">' 
									+ '<div style="font-weight:700; font-size:1.5em; align-self:flex-end; margin-right:5px;">' + Math.floor(Number(c.LTTimer)/60) + ' minutes ' + (Number(c.LTTimer) - ((Math.floor(Number(c.LTTimer)/60)) * 60)) + ' seconds</div>' 
							+ '</div></div>' : '') 
						+ '</div>';
					setuptools.app.muledump.tooltip(boost, 'mr5 autoHeight', boostHtml);
				}
				var portimg = $('<img class="portrait">');
				var CharText = $('<div>');
				CharText.append($('<div style="font-weight:700; font-size:1.1em; align-self:center"">').text(classname + ' lvl ' + c.Level), $('<div style="font-weight:700; font-size:1.1em; align-self:center">').text(NumberFormat(c.CurrentFame, ',') + ' Fame '));
				...
				if (c.Pet) {
					...
					var petimg = $('<img class="flex-container noFlexAutoWidth pet" style="flex-flow:column" src="lib/media/pets/' + PetName + '.png"></img>');
				...
				var CurrTF = NumberFormat(iTotalFame, ',', 3);
				var charHtml = '<div class="flex-container noFlexAutoWidth noFlexAutoAlign" style="color:#fff; flex-flow:column">' 
					+ '<div class="flex-container noFlexAutoAlign" style="justify-content:flex-start">' 
						+ '<div class="flex-container item noFlexAutoWidth" style="background-position: -' + FameItem[3] + 'px -' + FameItem[4] + 'px; border: solid 1px #000; align-self:center; flex-shrink:0; margin-right:5px;"></div>'
						+ '<div class="flex-container noFlexAutoWidth" style="flex-flow:column; flex-shrink:0; margin-right:5px">' 
							...
							+ '<div style="font-weight:700; font-size:1.15em; align-self:flex-start">Active Time : '
								+ '</div>' 
						+ '</div>' 
						+ '<div class="flex-container noFlexAutoAlign" style="flex-direction:column; margin-right:5px;">' 
					...
					+ '<div class="flex-container noFlexAutoAlign" style="justify-content:flex-start; margin-top:5px">'
						+ '<div class="flex-container item noFlexAutoWidth" style="background-position: -' + XPItem[3] + 'px -' + XPItem[4] + 'px; border: solid 1px #000; align-self:center; flex-shrink:0; margin-right:5px;"></div>'
						+ '<div class="flex-container noFlexAutoWidth" style="flex-flow:column; flex-shrink:0; margin-right:5px">' 
					...
					+ '</div>' 
				+ '</div>';
				setuptools.app.muledump.tooltip(CharText, 'mr5 autoHeight', charHtml);
				chtext.append(CharText);
				
				var petHtml = '<div class="flex-container noFlexAutoWidth noFlexAutoAlign" style="color:#fff; flex-flow:column">' 
					+ '<div class="flex-container noFlexAutoAlign mr5">' 
						+ '<div class="flex-container noFlexAutoWidth" style="border:solid 1px #000; margin-right:8px; align-self:center; background-color: #545454">' 
							+ '<img src="lib/media/pets/' + PetName + '.png"></img></div>'
					...
					+ '</div>'
				+ '</div>';
				setuptools.app.muledump.tooltip(petimg, 'mr5 autoHeight', petHtml);
 ...
 * @returns {jQuery}
 */
setuptools.app.muledump.item_listing = function(arr, classname, classid, line) {
    ...
    setuptools.lightbox.build('wardrobe-index', ' \
        <div class="flex-container" style="justify-content: space-between;">\
            <ul style="padding-left: 40px;">\
                ...
                <li>You possess ' + (count.owned-30) + ' of ' + (count.total-15) + ' skins (' + ( (((count.owned-30)/(count.total-15))*100).toFixed(0) ) + '%)</li>\
            </ul>\
