...
setuptools.app.mulequeue = {
    ...
    state: {
        ...
        rateLimited2: false,                     //  whether or not we are rate limited (Date.now() of rate limit detection)
...
setuptools.app.mulequeue.main = function() {
    ...
    //  check if we're already rate limited
    setuptools.app.mulequeue.state.rateLimitExpiration2 = setuptools.storage.read('ratelimitexpiration2');
    if ( !setuptools.app.mulequeue.state.rateLimitExpiration2 ) setuptools.app.mulequeue.state.rateLimitExpiration2 = Date.now();
    if ( setuptools.app.mulequeue.state.rateLimitExpiration2 > new Date ) setuptools.app.mulequeue.task.rateLimit2(true);
...
setuptools.app.mulequeue.task.background = function() {
    ...
    //  check for rate limiting
    if ( setuptools.app.mulequeue.state.rateLimited2 === true ) return;

    //  check for queue items
    if (
        setuptools.app.mulequeue.state.rateLimited === false &&
        setuptools.app.mulequeue.state.rateLimited2 === false &&
...
//  rate limit all tasks
setuptools.app.mulequeue.task.rateLimit2 = function(soft) {

    function RateLimitTimer2() {

        var RateLimitRemaining = ((setuptools.app.mulequeue.state.rateLimitExpiration2 - Date.now()) / 1000 / 60).toFixed(2);
		console.log('RateLimit2 = ' + setuptools.app.mulequeue.state.rateLimitExpiration2 + ' - ' + (((setuptools.app.mulequeue.state.rateLimitExpiration2 - Date.now()) / 1000 / 60).toFixed(2)))
        if (RateLimitRemaining <= 0) {

            clearInterval(setuptools.app.mulequeue.state.rateLimitTimer2);
            setuptools.app.mulequeue.state.rateLimitTimer2 = false;
            setuptools.app.mulequeue.state.rateLimited2 = false;
            $('div.mulequeue.progress > div.bar').removeClass('rateLimited nopulse');
            if ( setuptools.data.config.animations === 1 ) $('#stickynotice').removeClass('rateLimited nopulse');
            ...
            $('#mulequeue').removeClass('rateLimited');
            setuptools.app.mulequeue.ui.progressBar();
            window.techlog('MuleQueue/RateLimit2 expired', 'force');
    ...
    //  prepare the rate limit
    window.techlog('MuleQueue/RateLimit2 detected', 'force');
    setuptools.app.mulequeue.state.rateLimited2 = true; {

        // setuptools.app.mulequeue.state.rateLimitExpiration = Date.now()+setuptools.config.mqRateLimitExpiration;
        setuptools.app.mulequeue.state.rateLimitExpiration2 = Date.now()+setuptools.data.config.mqBGTimeout;

        //  write it to storage
        setuptools.storage.write('ratelimitexpiration2', setuptools.app.mulequeue.state.rateLimitExpiration2);
	}
    //  reset state for any tasks that encountered rate limiting
    if ( setuptools.app.mulequeue.queue.length > 0 ) {
        for (var index = (setuptools.app.mulequeue.state.busy - 1); index >= 0; index--) {
    ...
    //  start the rate limit timer
    setuptools.app.mulequeue.state.rateLimitTimer2 = setInterval(RateLimitTimer2, 1000);
...
setuptools.app.mulequeue.task.start = function(guid, callback) {
    ...
    setuptools.app.mulequeue.tasks[guid].promise = setuptools.app.mulequeue.task.createPromise(guid).then(function(response) {
        ...
        if (response.state === 'finished' && task.state === 'running' ) {
            ...
            //  handle rate limit detection
            if ( response.status === 'rateLimited2' ) {

                setuptools.app.mulequeue.task.rateLimit2();
                return;
...
setuptools.app.mulequeue.task.pause = function(callback) {
    ...
    if ( setuptools.app.mulequeue.state.rateLimited === false && setuptools.app.mulequeue.state.rateLimited2 === false ) $('#mulequeue').removeClass('running').addClass('paused');
...
setuptools.app.mulequeue.task.resume = function(guids, callback) {
    ...
    if ( guids.length === 0 ) {
        ...
        if ( setuptools.app.mulequeue.state.rateLimited === false && setuptools.app.mulequeue.state.rateLimited2 === false ) $('#mulequeue').removeClass('paused').addClass('running');
        setuptools.app.mulequeue.state.paused = false;
...
setuptools.app.mulequeue.menu = function() {
    ...
    if ( setuptools.app.mulequeue.state.rateLimited === true || setuptools.app.mulequeue.state.rateLimited2 === true ) {
...
setuptools.app.mulequeue.ui.manager = function(guid) {
    ...
    function createRow(listPos) {
		...
		if ( setuptools.app.mulequeue.state.rateLimited2 === true ) WaitTime.estimation = (WaitTime.estimation+Math.ceil((setuptools.app.mulequeue.state.rateLimitExpiration2-Date.now())/1000)+( (listPos > 0) ? AccountLoadDelay : 0 ));
    ...
    if ( setuptools.app.mulequeue.state.rateLimited2 === true ) rateLimited2 = 'rateLimited2';
...
setuptools.app.mulequeue.ui.progressBar = function(jobsRemaining, jobsTotal) {
    ...
    if ( setuptools.app.mulequeue.state.rateLimited2 === true ) {
        return;
    }
...
setuptools.app.mulequeue.spinner.running = function(type, returnHtml) {
    ...
    if ( type === 'queue' && setuptools.app.mulequeue.state.rateLimited2 === true ) type = 'error';
